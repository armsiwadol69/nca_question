...
if($_GET['id']){
    ...
    foreach($questioninfo AS $key => $val){
        ...
        if(!$val['questiondt_parent']){
            ...
            $htmlQuestion  .= $ncaquestion->generateIsParentQuestion("questiondt",$val['questiondt'],0);
...
<form class="needs-validation" action="../phpfunc/curd.php?method=addQuestion" enctype="multipart/form-data" method="POST" id="frm" novalidate>
...
<script>
    ...
    function generateInput(data) {
        ...
        html += `   <div class="list-group-item nested-3 question ms-2" id="`+name+`" data-id="`+name+`">`;
        ...
        html += `           <div class="list-group nested-sortable">`;
                ...
                for (let index = 0; index < number; index++) {
                    ...
                    // inp += `<input type="`+inptype+`" name="`+name+`" class="`+formctr+`" `+inputType+` >`;
                    inp += `<input type="text" name="`+name+`" class="form-control-50 col-lg-1" `+inputType+` value="`+inptype+`" readonly="readonly">`;
                    inp += ` <input class="form-control-50 col-lg-5" type="text" name="option`+name+`[]" id="option`+name+index+`" required="" `+$readonly+`>`;
                    inp += `<div class="list-group-item nested-3 hide question`+name+index+` `+(opques ? 'ms-3 mt-3 mb-3 ' : '')+`" data-id="li8La">`;
                    
                    inp += `</div>`;
                    //inp += `<span class="btn btn-primary mt-3 " id="add2question`+name+index+`"  onclick="setQuestionmodal('question','p');">เพิ่มคำถาม</span>`;
                    inp += `<span class="btn btn-primary ms-3" id="addquestion`+name+index+`" style="" onclick="setQuestionmodal('question','af','question`+name+index+`','`+name+`' ,'option`+name+index+`');">สร้างคำถามหลังจากคำตอบนี้</span>`;
                    // inp += `<span class="btn btn-primary mt-3" id="addQueSC`+name+`" style="display: none;" onclick="setQuestionmodal('question','p');">เพิ่มคำถาม</span>`;
                    html += inp;
                    html += `</div>`;
                }
