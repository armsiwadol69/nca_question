...
/* async function getRewardListDataFromAPI() {
	try {
		var endpoint = `../phpfunc/rewardapi.php?method=getRewardList`;
...
async function getPickUpDetail() {
	fireSwalOnSubmit();
	var pickupCode = document.getElementById("pickupCode").value;
	console.log(pickupCode);
	try {
		var endpoint = `../phpfunc/rewardapi.php?method=getPickUpDetail&pickupCode=${pickupCode}`;
		console.log(endpoint);
		const response = await axios.get(endpoint);
		console.log(response);
		var resData = response.data;
		console.log(resData);
		if (resData.resCode == "99") {
		fireSwal("info", "ไม่พบข้อมูล", "ไม่พบข้อมูลที่ตรงกับรหัสที่คุณกรอก");
		html = `<div class="alert alert-info text-center align-self-center fw-bold" role="alert">ไม่พบข้อมูล</div>`;
		document.getElementById("pickupDetail").innerHTML = html;
		document.getElementById("pickupConfrimBtn").setAttribute("hidden", "");
		return;
		}
		//document.getElementById('pickupDetail').innerHTML = JSON.stringify(resData);
		renderPickupDetail(resData.data[0]);
	} catch (error) {
		fireSwal("error", "พบข้อผิดพลาด", error);
	}
}
...
function renderPickupDetail(data) {
	document.getElementById("par_pickupCode").value = data.giftrecords_code;
	const html = `
	<table class="table shadow-sm table-striped table-bordered reward-info-table" id="pickupDetailTable">
		<tr>
			<td>รหัสรับของ : </td>
			<td class="fw-bold">${data.giftrecords_code}</td>
		</tr>
		<tr>
			<td>ชื่อผู้รับ : </td>
			<td>${data.giftrecords_usernm}</td>
		</tr>
		<tr>
			<td>สาขาที่รับ : </td>
			<td>${data.giftrecords_pickupoutletnm}</td>
		</tr>
		<tr>
			<td>วันที่กำหนดรับ : </td>
			<td>${dayjs(data.giftrecords_pickupdate, "YYYY-MM-DD").format("DD MMMM BBBB")}</td>
		</tr>
		<tr>
			<td>ชื่อของพรีเมี่ยม : </td>
			<td>${data.giftdetail_name}</td>
		</tr>
		<tr>
			<td>พาร์ทเนอร์ : </td>
			<td>${data.giftdetail_partnername}</td>
		</tr>
		<tr>
			<td>ประเภท : </td>
			<td>${renderType(data.giftrecords_type)}</td>
		</tr>
		<tr>
			<td>หมวดหมู่ : </td>
			<td>${data.giftcategory_name}</td>
		</tr>
		<tr>
			<td>สถานะ : </td>
			<td>${renderPickupStatus(data.giftrecords_iscomplete,data.giftrecords_type,data.giftrecords_pickupdate)}</td>
		</tr>
		<tr>
			<td>ดำเนินการโดย : </td>
			<td>${checkNullOrEmpty(data.giftrecords_modidsnm)}</td>
		</tr>
		<tr>
			<td>ดำเนินการเมื่อ : </td>
			<td>${checkNullOrEmpty(data.giftrecords_pickupdatetime)}</td>
		</tr>
	</table>
	<hr>`;
	document.getElementById("pickupDetail").innerHTML = html;
	const submitButton = document.getElementById("pickupConfrimBtn");
	if (data.giftrecords_iscomplete == "0") {
...
async function userRecriveGift() {
	...
	await axios
		.post("../phpfunc/rewardapi.php?method=pickupGift", formData)
		.then(function (response) {
		console.log(response);
		if (response.data.resCode == "1") {
			Swal.fire({
